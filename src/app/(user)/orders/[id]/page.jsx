...
export default function OrderDetailPage() {
  ...
  useEffect(() => {
    ...
    // 1. Fetch thông tin đơn hàng cụ thể
    fetch(`/api/orders`)
      .then(res => res.json())
      .then(data => {
        ...
        if (foundOrder) {
          ...
          // 2. Fetch thông tin vị trí/chi nhánh dựa trên location_id của đơn hàng
          fetch("/api/tables")
            .then(res => res.json())
            .then(tables => {
              const loc = Array.isArray(tables) ? tables.find(t => t._id === foundOrder.location_id) : null;
              setLocationName(loc ? loc.name : "Chi nhánh mặc định");
            })
            .catch(() => setLocationName("Không rõ địa điểm"));
        }
      })
      ...
      <div className="card shadow-sm border-0 rounded-4 overflow-hidden bg-white">
        ...
        <div className="p-4 border-bottom bg-light-subtle">
          ...
          <div className="row g-2 small">
            ...
            <div className="col-4 text-muted">Cửa hàng nhận:</div>
            <div className="col-8 fw-semibold text-dark">{locationName}</div>
        ...
        {/* Chân trang tính tiền */}
        <div className="p-4 bg-white d-flex justify-content-between align-items-center">
          <span className="fw-bold text-secondary">Tổng số tiền đã thanh toán:</span>
          <span className="h3 fw-bold text-danger mb-0">{order.total?.toLocaleString("vi-VN")}đ</span>
        </div>
      </div>
