...
import { useEffect, useState, useContext } from 'react'; // Thay đổi: Thêm useContext
import Link from 'next/link';
import { CartContext } from '@/components/CartContext'; // Thay đổi: Import CartContext đúng đường dẫn của bạn
...
export default function ProductDetailPage() {
    ...
    // Thay đổi: Lấy cart và setCart từ Context
    ...
    const handleAddToCart = () => {
        ...
        // Tạo bản sao của giỏ hàng hiện tại trong Context
        const newCart = [...cart];

        // Tìm sản phẩm trùng ID + cùng Size + cùng Màu
        ...
        if (existingIndex !== -1) {
            // Tăng số lượng nếu trùng thuộc tính
            newCart[existingIndex].quantity += quantity;
        } else {
            // Thêm sản phẩm mới với size và màu đã chọn
            newCart.push({
                _id: product._id,
                name: product.name,
                price: product.price,
        ...
        // Cập nhật trạng thái Context toàn cục (tự động lưu vào localStorage nếu Context của bạn đã cấu hình sẵn)
        setCart(newCart);
