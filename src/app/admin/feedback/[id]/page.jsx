  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        router.push("/admin/feedback"); // Sửa lại đường dẫn chuyển hướng về trang quản lý admin
      } else {
        alert(data.message);
      }
    } catch (error) {
  ...
  return (
    ...
    >
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          <h3 className="fw-bold">📩 Chi tiết Feedback</h3>
          <p className="text-secondary">Xem và phản hồi khách hàng</p>
        </div>

        {/* Sửa lại Link quay về đúng trang danh sách trong Admin */}
        ...
        <div className="card-body">
          <div className="row mb-3">
            <div className="col-md-6">
              ...
              <input className="form-control" value={feedback.name} disabled />
            </div>

            <div className="col-md-6">
              <label className="form-label fw-semibold">Email</label>
              <input className="form-control" value={feedback.email} disabled />
            </div>
          </div>

          <div className="mb-3">
            <label className="form-label fw-semibold">Chủ đề</label>
            <input className="form-control" value={feedback.subject || ""} disabled />
          </div>
          ...
          <div className="mb-4">
            ...
            <div>
              {feedback.status === "done" ? (
                ...
                <span className="badge bg-warning text-dark">Chưa xử lý</span>
              )}
            </div>
          </div>
          ...
          <div className="mb-4">
            <label className="form-label fw-semibold">Phản hồi khách hàng</label>
            <textarea
              className="form-control"
              rows={7}
