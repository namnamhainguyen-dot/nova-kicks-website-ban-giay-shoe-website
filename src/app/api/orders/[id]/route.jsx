import clientPromise from "../../../../libs/mongodb"; // Đường dẫn kết nối MongoDB của bạn
...
export async function GET(request, { params }) {
  try {
    ...
    return Response.json({ ...order, _id: String(order._id), success: true });
  } catch (error) {
...
// ==========================================
// ✅ 2. HÀM PATCH (Cập nhật trạng thái & Gửi Mail)
// ==========================================
export async function PATCH(request, { params }) {
  try {
    ...
    if (body.isPaid !== undefined) updateFields.isPaid = body.isPaid;
    if (body.status === "cancelled" && body.cancelReason) updateFields.cancelReason = body.cancelReason;

    if (Object.keys(updateFields).length === 0) {
      return Response.json({ success: false, error: "Không có dữ liệu cập nhật" }, { status: 400 });
    }

    // Tiến hành cập nhật trạng thái đơn hàng vào Database
